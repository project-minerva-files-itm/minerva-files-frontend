...
const useApiDepartment = () => {
  ...
  const get = async (page: number, record: number) => {
    ...
    return await apiClient.get<ApiResponse<Department>>(
      `${apiUrl}/${endpoint}/${query}`
    );
  };

  const save = async (data: Department) => {
    return await apiClient.post<ApiResponse<Department>>(
      `${apiUrl}/${endpoint}`,
      data
    );
  };
  ...
  const update = async (data: Department) => {
    return await apiClient.put<ApiResponse<Department>>(
      `${apiUrl}/${endpoint}`,
      data
    );
  };
  ...
  const deleted = async (id: number) => {
    return await apiClient.delete<ApiResponse<Department>>(
      `${apiUrl}/${endpoint}/${id}`
    );
  };
